...
export default function InchargeDashboardPage() {
	...
	const { data: rosterData } = useQuery({
		...
		queryFn: async () =>
			(await api.get("/attendance/incharge")).data.data as {
				students?: unknown[];
				isWaived?: boolean;
			},
	});
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div className="p-4 rounded-lg border border-slate-100 bg-slate-50">
						...
						<p className="text-2xl font-bold text-slate-900">
							{Array.isArray(rosterData?.students)
								? rosterData.students.length
								: 0}
						</p>
						<p className="text-xs text-slate-500 mt-1">
							Currently eating in this mess
						</p>
					</div>
					<div
						className={`p-4 rounded-lg border ${rosterData?.isWaived ? "border-amber-200 bg-amber-50" : "border-slate-100 bg-slate-50"}`}
					>
						<p className="text-sm font-semibold text-slate-800 mb-1">
							Mess Day Status
						</p>
						<p
							className={`text-2xl font-bold ${rosterData?.isWaived ? "text-amber-700" : "text-green-700"}`}
						>
							{rosterData
								? rosterData.isWaived
									? "Waived"
						...
						<p className="text-xs text-slate-500 mt-1">
							{rosterData?.isWaived
								? "No charges applied for today."
								: "Charges applied as normal."}
						</p>
					</div>
				</div>
			</div>
