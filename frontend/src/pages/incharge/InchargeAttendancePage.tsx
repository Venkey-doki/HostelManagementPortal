...
type AttendanceDraft = Record<string, { breakfast: boolean; lunch: boolean; dinner: boolean }>;
...
export default function InchargeAttendancePage() {
	...
	const loadRoster = async () => {
		...
		try {
			const res = await api.get("/attendance/incharge", { params: { date } });
			const data = res.data.data as InchargeRosterResponse;
			setRoster(data);
			setDraft(Object.fromEntries(data.students.map((s) => [s.studentId, { ...s.attendance }])));
			setWaiverReason(data.waiver?.reason ?? "");
		} catch (err: any) {
			setError(err.response?.data?.error?.message ?? "Failed to load attendance roster");
		} finally { setLoading(false); }
	};

	useEffect(() => { void loadRoster(); }, [date]); // eslint-disable-line react-hooks/exhaustive-deps
	...
	const updateMeal = (sid: string, meal: "breakfast" | "lunch" | "dinner", val: boolean) => {
		setDraft((c) => ({ ...c, [sid]: { ...(c[sid] ?? { breakfast: false, lunch: false, dinner: false }), [meal]: val } }));
	};
	...
	const saveSingle = async (sid: string) => {
		...
		try {
			const row = draft[sid];
			if (!row) return;
			await api.patch("/attendance/incharge/mark", { studentId: sid, date, ...row });
			setSuccess("Attendance updated.");
		} catch (err: any) { setError(err.response?.data?.error?.message ?? "Failed to update attendance"); }
	...
	const saveBulk = async () => {
		...
		try {
			await api.patch("/attendance/incharge/bulk", {
				date,
				rows: roster.students.map((s) => ({ studentId: s.studentId, ...(draft[s.studentId] ?? { breakfast: false, lunch: false, dinner: false }) })),
			});
			setSuccess("Bulk attendance saved.");
			await loadRoster();
		} catch (err: any) { setError(err.response?.data?.error?.message ?? "Failed to bulk save attendance"); }
		finally { setSaving(false); }
	};
	...
	const setWaiver = async () => {
		setSaving(true); setError(""); setSuccess("");
		try {
			await api.post("/attendance/incharge/waivers", { date, reason: waiverReason || undefined });
			setSuccess("Mess-day waiver set.");
			await loadRoster();
		} catch (err: any) { setError(err.response?.data?.error?.message ?? "Failed to set waiver"); }
		finally { setSaving(false); }
	};
	...
	const clearWaiver = async () => {
		setSaving(true); setError(""); setSuccess("");
		try {
			await api.delete("/attendance/incharge/waivers", { params: { date } });
			setSuccess("Mess-day waiver removed.");
			await loadRoster();
		} catch (err: any) { setError(err.response?.data?.error?.message ?? "Failed to remove waiver"); }
		finally { setSaving(false); }
	};
		...
		<div className="space-y-6">
			...
			<div className="flex items-start justify-between gap-4 flex-wrap">
				<div>
					...
					<p className="mt-0.5 text-sm text-slate-500">
						Mark attendance meal-wise. Backdating is allowed. Set a waiver for mess holidays.
					</p>
				</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<div className="bg-white rounded-lg border border-slate-200 p-5">
					<h2 className="text-sm font-semibold text-slate-900 mb-3">Select Date</h2>
					<input type="date" max={new Date().toISOString().slice(0, 10)} value={date} onChange={(e) => setDate(e.target.value)} className={inputClass} />
					<p className="mt-2 text-xs text-slate-400">Future dates are not allowed. Backdating to past days is permitted.</p>
				</div>
				...
				<div className="bg-white rounded-lg border border-slate-200 p-5">
					...
					{isWaived && (
						<p className="text-xs text-amber-700 mb-3">Waived for {roster?.waiver?.date}{roster?.waiver?.reason ? ` · ${roster.waiver.reason}` : ""}</p>
					)}
					<input type="text" value={waiverReason} onChange={(e) => setWaiverReason(e.target.value)}
						placeholder="Waiver reason (holiday / event)…" className={inputClass} />
					<div className="flex gap-2 mt-3">
						<button type="button" onClick={() => void setWaiver()} disabled={saving}
						...
						<button type="button" onClick={() => void clearWaiver()} disabled={saving || !isWaived}
			...
			<div className="grid grid-cols-3 gap-4">
				{[{ label: "Breakfast", value: totals.breakfast }, { label: "Lunch", value: totals.lunch }, { label: "Dinner", value: totals.dinner }].map((s) => (
					<div key={s.label} className="bg-white rounded-lg border border-slate-200 p-4 text-center">
						<p className="text-xs font-medium text-slate-500 uppercase tracking-wide">{s.label} marked</p>
			...
			<div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
				<div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3">
					<h2 className="text-sm font-semibold text-slate-900">Daily Roster</h2>
					<div className="flex gap-2">
						{loading && <span className="text-xs text-slate-400">Loading…</span>}
						<button type="button" onClick={() => void loadRoster()} disabled={loading || saving}
						...
						<button type="button" onClick={() => void saveBulk()} disabled={loading || saving || isWaived}
				...
				{!loading && !roster?.students.length ? (
					<p className="text-center py-10 text-sm text-slate-400">No students assigned to this mess for the selected date.</p>
				) : (
					<div className="overflow-x-auto">
						<table className="w-full text-sm">
							<thead className="bg-slate-50 border-b border-slate-200">
								<tr>
									{["Student", "Roll No.", "Breakfast", "Lunch", "Dinner", ""].map((h) => (
									...
									<tr key={s.studentId} className="hover:bg-slate-50">
										...
										{(["breakfast", "lunch", "dinner"] as const).map((meal) => (
											<td key={meal} className="px-4 py-3">
												<input
													type="checkbox"
													checked={draft[s.studentId]?.[meal] ?? false}
													onChange={(e) => updateMeal(s.studentId, meal, e.target.checked)}
										...
										<td className="px-4 py-3">
											<button type="button" onClick={() => void saveSingle(s.studentId)} disabled={isWaived || saving}
